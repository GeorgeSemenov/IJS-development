set parentName=onepage-pagination
...
if /I %isThereNeedPugFile% NEQ %zero% (
  ...
  (
  echo include %filename%/onepage-pagination%filename%.pug
  )> newFile.pug
  type onepage-pagination.pug >> newFile.pug
  move /Y newFile.pug  onepage-pagination.pug
  (
...
(
echo @import '%fileName%/onepage-pagination%fileName%';
)>>onepage-pagination.scss
cd %fileName
(
...
if  /I %isThereElement% == %True% (
  (
  echo @echo off
  echo set grandParentName=onepage-pagination
  echo set parentName=%fileName%
  echo set /p fileName=What would you create?
  echo set newStyleSCSS=onepage-pagination%fileName%.scss
  echo mkdir %%fileName%%
  echo ^(
  echo echo @import '%%fileName%%/onepage-pagination%fileName%%%fileName%%';
  echo ^)^>^>%%newStyleSCSS%%
  echo cd %%fileName%%
  echo ^(
